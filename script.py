...
class EnhancedANITrainer:
    ...
    def train(self, train_data, val_data, early_stop=True, draw_curve=True):
        ...
        if draw_curve:
            ...
            #save my training curve 
            plt.savefig('training_curve_rmse.png')
            plt.close()
...
# Enhanced trainer with anti-overfitting settings
trainer = EnhancedANITrainer(
    model=model,
    batch_size=32,
    learning_rate=1e-3,
    epoch=100,
    l2=1e-4,                 # Stronger L2 regularization
    lr_schedule='step',      # Use step decay schedule
    lr_step_size=15,         # Reduce learning rate every 15 epochs
    lr_gamma=0.1,            # By factor of 0.1
    gradient_clip_val=0.5,   # Clip gradients to prevent exploding gradients
    #early_stop_patience=10,  # Wait 10 epochs before early stopping
    add_noise=True,          # Use coordinate noise as data augmentation
    noise_level=0.005,       # Small amount of noise
    validation_split=0.15    # Proper validation set size
)

# Train the model with all anti-overfitting techniques
...
torch.save(model.state_dict(), 'trained_model_rmse_final.pth')
print("Model saved to trained_model_rmse_final.pth")